...
ActiveRecord.inherit = function(class)
  ...
  class.listEach = function(params)
    ...
    if empty(params.order) then
      if reverse then
        params.order = table .. "."  .. key .. " DESC"
        operator = "<"
      else
        params.order = table .. "."  .. key .. " ASC"
      end
    end
  ...
  class.sum = function(column, params)
    return class.adapter():sum(column, params or {})
  end
