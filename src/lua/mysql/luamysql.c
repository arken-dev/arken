...
static int l_new_mysqlclient(lua_State* l)
{
    ...
    //lua_setmetatable(l, MySQLLib); //old
...
static int l_mysqlclient_ping(lua_State* l)
{
    ...
    conn = luaL_checkudata(l, 1, MySQLLib);
    if (!conn) {
...
static int l_mysqlclient_query(lua_State* l)
{
    ...
    if (err) {
        ...
        lua_setmetatable(l, -2);
        //lua_setmetatable(l, MySQLResult); //old
...
{
    MYSQL* conn = luaL_checkudata(l, 1, MySQLLib);
    if (conn)
        mysql_close(conn);
...
{
    MYSQL_RES** result = luaL_checkudata(l, 1, MySQLResult);
