...
static int cur_iter(lua_State *L) {
	cur_data *cur = (cur_data *) luaL_checkpointer(L, lua_upvalueindex(1));
	PGresult *res = cur->pg_res;
	int tuple = cur->curr_tuple;
	...
	if (lua_istable (L, lua_upvalueindex(2))) {
		...
		return 1; /* return #numcols values */
	}
}
...
static int cur_each(lua_State *L) {
        int n = 1;
	cur_data *cur = getcursor (L);
