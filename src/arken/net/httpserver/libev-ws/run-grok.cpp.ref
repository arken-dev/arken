...
//
// Correções de alta concorrência aplicadas:
// - client_number agora é std::atomic (elimina data race)
// - sockets aceitos são colocados em non-blocking
// - writeAll trata EAGAIN/EWOULDBLOCK sem travar o event loop
// - limite de tamanho no buffer de entrada WebSocket
// - SO_REUSEPORT habilitado (quando disponível)
...
/* limite máximo do buffer de entrada WebSocket (proteção contra DoS de memória) */
#define MAX_WS_INPUT_SIZE (1 * 1024 * 1024)   // 1 MiB
...
/* record the number of clients — atomic para eliminar data race entre threads */
...
// writeAll defensivo: não trava o event loop em EAGAIN/EWOULDBLOCK.
// Ideal a médio prazo: buffer de saída por Connection + watcher EV_WRITE.
static void
writeAll(int fd, const char * data, size_t size)
{
  size_t total = 0;
  while (total < size) {
    ssize_t bytes = write(fd, data + total, size - total);
    if (bytes > 0) {
      total += static_cast<size_t>(bytes);
      continue;
    }
    if (bytes == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
      // socket ainda não está pronto para escrita.
      // Em produção o ideal é pausar e registrar EV_WRITE.
      // Por enquanto apenas desiste para não travar o loop.
      fprintf(stderr, "arken.net.HttpServer (libev-ws): write would block (fd=%d)\n", fd);
      return;
    }
    // erro real
    fprintf(stderr, "arken.net.HttpServer (libev-ws): write error (fd=%d): %s\n",
            fd, strerror(errno));
    return;
  }
}
...
create_serverfd(char const *addr, uint16_t port)
{
  ...
  if(setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval)) == -1) {
    std::cerr << "setsockopt SO_REUSEADDR fail\n";
    throw;
  }

#ifdef SO_REUSEPORT
  // melhora o balanceamento de accepts entre threads e reduz thundering herd
  if(setsockopt(fd, SOL_SOCKET, SO_REUSEPORT, &optval, sizeof(optval)) == -1) {
    std::cerr << "setsockopt SO_REUSEPORT fail\n";
    // não é fatal em kernels antigos — continua com SO_REUSEADDR apenas
  }
#endif
  ...
  // set nonblock flag no socket de escuta
  if( fd > 0 ) {
...
read_cb(struct ev_loop *loop, struct ev_io *watcher, int revents)
{
  ...
  // proteção contra crescimento descontrolado do buffer no caminho WebSocket
  if( connection->type == ConnectionType::WEBSOCKET &&
      connection->input.size() > MAX_WS_INPUT_SIZE ) {
    fprintf(stderr, "arken.net.HttpServer (libev-ws): WebSocket input too large (%zu bytes), closing\n",
            connection->input.size());
    WebSocketRegistry::writeFrame(connection->sessionId,
                                  WebSocketParser::buildClose(1009, "too large"));
    closeConnection(loop, connection);
    return;
  }
...
accept_cb(struct ev_loop *loop, struct ev_io *watcher, int revents)
{
  ...
  if (connfd > 0) {
    // fetch_add retorna o valor *antes* do incremento
    if (client_number.fetch_add(1) >= MAX_CLIENTS) {
      close(connfd);
      client_number.fetch_sub(1);
    } else {
      // torna o socket aceito non-blocking (evita travar o event loop em write)
