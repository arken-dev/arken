...
test.should_return_controllerName_and_layout = function()
  ...
  assert( status == 200, status )
  assert( headers[1] == 'Content-Type: text/html', headers[1] )
  assert( body:remove('\n') == "<html><body>teste<br><h3>Order</h3></body></html>", body )
end

test.should_return_value_and_layout = function()
  local c = Controller.new{ controllerName = 'order', actionName = 'index' }
  c.layout = 'test'
  local status, headers, body = c:render({ output = 'html', value = '<h3>Value</h3>', layout = 'test' })
  assert( status == 200, status )
  assert( headers[1] == 'Content-Type: text/html', headers[1] )
  assert( body:remove('\n') == "<html><body>teste<br><h3>Value</h3></body></html>", body )
end
...
test.should_return_view_and_layout = function()
  ...
  assert( status == 200, status )
  assert( headers[1] == 'Content-Type: text/html', headers[1] )
  assert( body:remove('\n') == '<html><body>teste<br><h3>Order</h3></body></html>', body:escapeHtml() )
end
