...
test['should return __gads'] = function()
  local buffer = os.read(ARKEN_PATH .. "/tests/lib/arken/net/cookie/1-example.cookie")
...
test['return one value _session_id'] = function()
  local buffer = os.read(ARKEN_PATH .. "/tests/lib/arken/net/cookie/2-example.cookie")
  local result = cookie.parse(buffer)
