...
-- janela de `seconds` segundos (default: 60). exceeded(ip) devolve true
...
-- enxergam o mesmo contador -- por isso a chamada é `limiter.exceeded(ip)`,
...
for i = 1, 4 do
  local blocked = limiter.exceeded(ip)
  print(string.format('hit %d -> exceeded() = %s', i, tostring(blocked)))
end

-- login OK (ou qualquer evento que "perdoa" o IP): limpa o contador
limiter.clear(ip)
print('after clear -> exceeded() = ' .. tostring(limiter.exceeded(ip)))

-- opcional: exceeded() já é varrido sozinho por uma thread de faxina
